...
async function restoreMockupSamplesIfEmpty() {
  try {
    ...
    const itemKey = (item: any) => item?.id || item?.sampleId || "";
    const mergeMissing = (current: any[], incoming: any[]) => {
    ...
    const nextCategories = categoriesCurrent.length
      ? mergeMissing(categoriesCurrent, BUILT_IN_MOCKUP_RESTORE_DATA.categories)
      : BUILT_IN_MOCKUP_RESTORE_DATA.categories;
    const nextPrompts = promptsCurrent.length
      ? mergeMissing(promptsCurrent, BUILT_IN_MOCKUP_RESTORE_DATA.prompts)
      : BUILT_IN_MOCKUP_RESTORE_DATA.prompts;
    let changed = false;
