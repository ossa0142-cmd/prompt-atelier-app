...
const sampleVideos: VideoItem[] = [
  {
    "id": "1783865358542-e8d7e67e028438",
    "folder": "未分類",
    "title": "女性",
    "url": "/samples/videos/pa-video-sample-woman.mp4",
    "model": "Runway",
    "thumbnail": "",
    "thumbnailMode": "video",
    ...
    "tags": [],
    "favorite": false,
    "createdAt": "2026-07-12T14:09:18.541Z",
    "updatedAt": "2026-07-12T14:09:18.541Z"
  }
];
...
function JournalPage({ journal, setJournal, setScreen }: any) {
  ...
  const savedCollages = journal.savedCollages || [];
  ...
  const [selectedCollageId, setSelectedCollageId] = React.useState("");
  const activeSavedCollageId = savedCollages.some((collage: JournalCollage) => collage.id === selectedCollageId) ? selectedCollageId : "";
  ...
  const adjustItemScale = (id: string, delta: number) => {
    const target = journal.items.find((item: JournalItem) => item.id === id);
    const nextScale = Math.min(2.6, Math.max(0.35, Number(((target?.scale || 1) + delta).toFixed(2))));
    updateItem(id, { scale: nextScale });
  };
  const adjustItemRotate = (id: string, delta: number) => {
    const target = journal.items.find((item: JournalItem) => item.id === id);
    const nextRotate = Math.min(180, Math.max(-180, (target?.rotate || 0) + delta));
    updateItem(id, { rotate: nextRotate });
  };
  const saveCurrentCollage = () => {
    const defaultTitle = `コラージュ ${savedCollages.length + 1}`;
    const title = window.prompt("保存名を入力してください", defaultTitle);
    if (title === null) return;
    const now = new Date().toISOString();
    const nextCollage: JournalCollage = {
      id: uid(),
    ...
    setJournal((current: JournalState) => ({
      ...current,
      savedCollages: [nextCollage, ...(current.savedCollages || [])].slice(0, 30),
    }));
    setSelectedCollageId(nextCollage.id);
  };
  const openSavedCollage = (id: string) => {
    if (!id) return;
    setJournal((current: JournalState) => {
  ...
  const deleteSavedCollage = (id: string) => {
    if (!id || !window.confirm("保存したコラージュを削除しますか？")) return;
    setJournal((current: JournalState) => ({
      ...current,
      savedCollages: (current.savedCollages || []).filter((item: JournalCollage) => item.id !== id),
    }));
    ...
    <section className="page journal-page">
      ...
      <div className="journal-layout">
        <aside className="journal-tools">
          ...
          <div className="journal-collage-panel">
            ...
            {savedCollages.length ? (
              <>
                <label>保存済みコラージュ
                  <select value={activeSavedCollageId} onChange={(event) => setSelectedCollageId(event.target.value)}>
                    <option value="">選択してください</option>
                    {savedCollages.map((collage: JournalCollage) => <option key={collage.id} value={collage.id}>{collage.title}</option>)}
                  </select>
                </label>
                <div className="journal-collage-actions">
                  <button type="button" disabled={!activeSavedCollageId} onClick={() => openSavedCollage(activeSavedCollageId)}>開く</button>
                  <button type="button" className="danger" disabled={!activeSavedCollageId} onClick={() => deleteSavedCollage(activeSavedCollageId)}>削除</button>
                </div>
              </>
            ) : (
              <small>保存したコラージュを後から開いて再編集できます</small>
            )}
          </div>
        ...
        <div className="journal-canvas">
          ...
          >
            {journal.items.map((item: JournalItem, index: number) => (
              <div
                className={`journal-sticker ${selectedId === item.id ? "selected" : ""}`}
                key={item.id}
                style={{ left: item.x, top: item.y, width: item.width, zIndex: index + 1, transform: `rotate(${item.rotate}deg) scale(${item.scale || 1})` }}
                onPointerDown={(event) => {
                  event.preventDefault();
                  setSelectedId(item.id);
              ...
              >
                ...
                {selectedId === item.id && (
                  <div className="journal-transform-controls" onPointerDown={(event) => event.stopPropagation()}>
                    <button type="button" aria-label="左に回転" onClick={() => adjustItemRotate(item.id, -8)}>↺</button>
                    <button type="button" aria-label="縮小" onClick={() => adjustItemScale(item.id, -0.1)}>−</button>
                    <button type="button" aria-label="拡大" onClick={() => adjustItemScale(item.id, 0.1)}>＋</button>
                    <button type="button" aria-label="右に回転" onClick={() => adjustItemRotate(item.id, 8)}>↻</button>
                  </div>
                )}
              </div>
            ))}
